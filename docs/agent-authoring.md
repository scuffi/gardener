...
For persistent effects, effective authority is the most restrictive of the workspace policy ceiling, repository policy, revision effect ceiling, and assignment authority ceiling. Snapshot authority is intersected with live state, so narrowing applies immediately and later widening never upgrades an in-flight run. Trigger eligibility and an enabled exact-repository assignment admit the Agent run independently of effect authority. Missing or partial repository policy produces a fail-closed, all-disabled snapshot: the Flue turn and proposal remain visible, but no persistent effect can execute. Deterministically corrupt policy is audited and rejected rather than interpreted. Workspace-local capabilities are evaluated against the revision plus workspace/repository capability policy; the assignment's persistent-effect authority ceiling does not gate them.
