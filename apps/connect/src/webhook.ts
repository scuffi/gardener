...
const ACTIONS: Readonly<Record<string, ReadonlySet<string>>> = {
  ...
  pull_request_comment: new Set(["created", "edited", "deleted"]),
...
export function normalizeGitHubWebhook(eventName: string, payloadValue: unknown, deliveryId: string, instanceId = "pending"): RepositoryEventV2 | null {
  ...
  } else if (eventName === "pull_request_comment") {
    const parent = pull(payload.pull_request), resource = comment(payload.comment), author = record(payload.comment) ? identity(payload.comment.user) : null;
    if (parent && resource && author) candidate = { ...base(payload, deliveryId, instanceId, resource.updatedAt, repo, actor, author), kind: "github.pull_request_comment", action: rawAction, pullRequest: parent, comment: resource };
