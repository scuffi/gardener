...
Central Cloudflare Worker for Gardener's managed GitHub boundary. It keeps GitHub App credentials and installation tokens out of customer Workers and exposes only signed identities/events, scoped run grants, and typed issue mutations.
...
2. Give the GitHub App **metadata: read** and **issues: write** permissions and subscribe to installation, installation repositories, and issues events.
...
- `POST /github/webhook`: verifies HMAC over raw bytes, deduplicates delivery IDs, normalizes issue events, signs them, and relays `{ "token": "..." }` to the claimed callback.
- `POST /v1/grants` (instance bearer): creates a five-minute grant for one run, delivered event, repository, issue, and operation set. Connect refuses grants that do not reference a matching event it signed and relayed to that instance.
- `POST /v1/operations` (grant bearer): executes only strict `v1` label/comment/close/reopen operations. Connect re-fetches state, mints a one-repository installation token internally, and records stable operation receipts.
