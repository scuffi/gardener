...
export function renderAsciiGardenScene({
  columns,
  rows,
  timeMs,
  seed,
  depth = 1,
  pointer,
}: AsciiGardenOptions): AsciiGardenScene {
  ...
  const bloomFields = Array.from({ length: 4 }, () =>
  ...
  if (depth > .72) for (let column = 2; column < width - 2; column += 1) {
    ...
    const plantHeight = Math.min(
      height - 3,
      minimumHeight + Math.floor(unit(seed, column, 89) * (maximumHeight - minimumHeight + 1)),
    );
    ...
    if (kind === "flower") {
      ...
      for (let patternRow = 0; patternRow < pattern.length; patternRow += 1) {
        for (let patternColumn = 0; patternColumn < pattern[patternRow]!.length; patternColumn += 1) {
          ...
          if (glyph !== " ") {
            const bloomRow = tip.row - 1 + patternRow;
            putBloom(bloomRow, tip.column - patternCenter + patternColumn, glyph, tone);
          }
        }
      }
...
export function AsciiGarden() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number, still = false) => {
      ...
      const farPointer =
        activePointer ?
          { pointer: { ...activePointer, strength: activePointer.strength * .45 } }
        : {};
      far.textContent = renderAsciiGarden({
        ...dimensions,
        timeMs,
        seed: farSeed,
        depth: .45,
      ...
      const nearScene = renderAsciiGardenScene({
        ...dimensions,
        timeMs,
        seed: nearSeed,
        depth: 1,
        ...(activePointer ? { pointer: activePointer } : {}),
      });
    ...
    const move = (event: PointerEvent) => {
      const unsupportedPointer =
        event.pointerType && event.pointerType !== "mouse" && event.pointerType !== "pen";
      if (reduced() || !finePointer.matches || unsupportedPointer) return;
      const bounds = root.getBoundingClientRect();
      const outside =
  ...
  const layer = "signin-garden__layer";
  const bloom = `${layer} ${layer}--bloom`;

  return (
    <div ref={rootRef} className="signin-garden" aria-hidden="true">
      <pre ref={farRef} className={`${layer} ${layer}--far`} />
      <pre ref={nearRef} className={`${layer} ${layer}--near`} />
      <pre ref={roseBloomRef} className={`${bloom} ${layer}--rose`} />
      <pre ref={goldBloomRef} className={`${bloom} ${layer}--gold`} />
      <pre ref={violetBloomRef} className={`${bloom} ${layer}--violet`} />
      <pre ref={blueBloomRef} className={`${bloom} ${layer}--blue`} />
    </div>
  );
}
