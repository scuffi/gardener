...
describe("pinned v2 issue execution", () => {
  it("loads the run revision after newer activation and applies the immutable proposal ceiling", async () => {
    ...
    try {
      ...
      const old = await compileWorkflowV2(spec("Pinned revision one instructions.", ["issue.label.add"]), {
      ...
      const current = await compileWorkflowV2(spec("Newer active instructions.", ["issue.label.add", "issue.comment.create"]), {
        workflowId: "pinned-workflow", revision: 2, resolvedModel: "pinned-model-v2", now: () => new Date("2026-09-03T11:00:00.000Z"),
      });
      sqlite.prepare(
      ...
      ).run("pinned-workflow", "Pinned workflow", 2, 1, "github.issue", "Newer mutable projection.", canonicalJson(current.plan), 2, 2);
      ...
      insertRevision.run("pinned-workflow", 2, canonicalJson(current.definition), canonicalJson(current.plan), current.definition.contentHash);
      ...
      expect(calls[0]?.input.messages[0].content).toContain("Pinned revision one instructions.");
      expect(calls[0]?.input.messages[0].content).not.toContain("Newer active instructions.");
      ...
      expect(calls[1]?.model).toBe("mutable-deployment-model");
      expect(calls[1]?.input.messages[0].content).toContain(legacyInstructions);
      expect(calls[1]?.input.messages[0].content).not.toContain("Backfilled replacement instructions.");
