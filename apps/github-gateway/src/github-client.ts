...
import { availableGitHubOperationKinds } from "@gardener/provider-github";
...
function headers(token: string): HeadersInit {
  return { authorization: `Bearer ${token}`, accept: "application/vnd.github+json", "x-github-api-version": "2022-11-28", "user-agent": "gardener-github-gateway", "content-type": "application/json" };
}
...
async function jsonResponse(response: Response, context: string): Promise<unknown> {
  if (!response.ok) {
    ...
    throw new GitHubOperationError(
      status === 429 ? "github_rate_limited" : "github_http_error",
...
export async function exchangeOAuthCode(env: Env, code: string): Promise<{ id: string; login: string }> {
  const exchange = await fetch("https://github.com/login/oauth/access_token", {
    method: "POST", headers: { accept: "application/json", "content-type": "application/json", "user-agent": "gardener-github-gateway" },
    body: JSON.stringify({
      client_id: env.GITHUB_CLIENT_ID,
      client_secret: env.GITHUB_CLIENT_SECRET,
      code,
      redirect_uri: new URL("/oauth/github/callback", env.GATEWAY_ORIGIN).toString(),
    }),
...
export async function verifyGitHubAppCredentials(env: Env): Promise<boolean> {
  const app = await jsonResponse(await github("/app", await appJwt(env)), "GitHub App verification");
  return record(app) && String(app.id) === env.GITHUB_APP_ID && typeof app.slug === "string" &&
    app.slug === env.GITHUB_APP_SLUG;
}
...
export async function getInstallation(env: Env, installationId: string): Promise<InstallationInfo> {
  ...
  if (
    ...
    !record(data.account) ||
    !positiveInteger(data.account.id) ||
...
async function installationWideToken(
  env: Env,
  installationId: string,
  permissions: RepositoryPermissions,
): Promise<string> {
  const data = await jsonResponse(await github(
    `/app/installations/${installationId}/access_tokens`,
    await appJwt(env),
    { method: "POST", body: JSON.stringify({ permissions }) },
  ), "Installation token request");
  if (!record(data) || typeof data.token !== "string" || !data.token) {
    throw new Error("Installation token response was invalid");
  }
  return data.token;
}
...
export async function resolveGitHubUsername(
  ...
  const token = await installationWideToken(env, installationId, { metadata: "read" });
  const response = await github(`/users/${encodeURIComponent(login)}`, token);
  if (response.status === 404) {
    await response.body?.cancel();
    return null;
  }
  const value = await jsonResponse(response, "GitHub user lookup");
  if (!record(value) || !positiveInteger(value.id) || typeof value.login !== "string") {
...
export async function discoverRepositories(env: Env, installationId: string): Promise<DiscoveredRepository[]> {
  ...
  throw new Error("Repository discovery exceeded the 10,000 repository safety limit");
}
...
const IMPLEMENTED_OPERATION_KINDS = new Set<Operation["kind"]>(availableGitHubOperationKinds);

function operationPermissions(operation: Operation): RepositoryPermissions {
  if (!IMPLEMENTED_OPERATION_KINDS.has(operation.kind)) {
    throw new GitHubOperationError(
...
async function commitMarker(env: Env, operation: Extract<Operation, { kind: "commit.create" }>): Promise<string> {
  const key = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(env.OPERATION_MARKER_KEY),
...
async function findPaginated(
  ...
  throw new Error(`${label} exceeded the 10,000-resource idempotency scan without an exact match`);
}
...
async function executeIssueOperation(env: Env, operation: IssueOperation, token: string): Promise<OperationResult> {
  ...
  if (operation.kind === "issue.comment.create") {
    ...
    if (!hasExactOperationMarker(operation.body, operation.id)) {
      throw new GitHubOperationError(
        ...
        "Host-constructed comment is missing its exact operation marker",
      );
    }
    const comment = await findPaginated(
      `${issuePath}/comments?sort=created&direction=desc`,
      token,
      "Comment idempotency lookup",
      (candidate) => authoredByApp(candidate, env) && candidate.body === operation.body,
    );
    if (comment) return { status: "already-applied", ...(positiveInteger(comment.id) ? { githubId: comment.id } : {}), ...(typeof comment.html_url === "string" ? { url: comment.html_url } : {}) };
  }
  ...
  if (operation.kind === "issue.label.add" || operation.kind === "issue.label.remove") {
    ...
    if (!response.ok) {
      throw new GitHubOperationError(
        response.status === 429 ? "github_rate_limited" : "github_http_error",
        `GitHub label mutation failed (${response.status})`,
  ...
  if (operation.kind === "issue.close" || operation.kind === "issue.reopen") {
    if (issue.state === desired) return { status: "already-applied" };
    if (issue.state !== operation.expectedIssueState) throw new Error(`Precondition failed: issue state is ${String(issue.state)}`);
    if (!sameInstant(issue.updated_at, operation.expectedIssueUpdatedAt)) throw new Error("Precondition failed: issue changed after the operation was approved");
  ...
  if (issue.state !== operation.expectedIssueState) throw new Error(`Precondition failed: issue state is ${String(issue.state)}`);
  if (!sameInstant(issue.updated_at, operation.expectedIssueUpdatedAt)) throw new Error("Precondition failed: issue changed after the operation was approved");
  return parseCommentResult(await jsonResponse(await github(`${issuePath}/comments`, token, { method: "POST", body: JSON.stringify({ body: operation.body }) }), "Comment creation"));
}
...
async function executeReview(env: Env, operation: Extract<Operation, { kind: "pull_request.review.submit" }>, token: string): Promise<OperationResult> {
  const repoPath = repositoryPath(operation);
  if (!hasExactOperationMarker(operation.body, operation.id)) {
    throw new GitHubOperationError("canonical_marker_missing", "Exact review body is missing its operation marker");
  }
  const expectedState = operation.event === "approve" ? "APPROVED" : operation.event === "request_changes" ? "CHANGES_REQUESTED" : "COMMENTED";
  const review = await findPaginated(
    `${repoPath}/pulls/${operation.pullNumber}/reviews`,
    token,
    "Review idempotency lookup",
    (candidate) => authoredByApp(candidate, env)
      && candidate.state === expectedState
      && candidate.commit_id === operation.expectedHeadSha
      && candidate.body === operation.body,
  );
  if (review) return { status: "already-applied", ...(positiveInteger(review.id) ? { githubId: review.id } : {}), ...(typeof review.html_url === "string" ? { url: review.html_url } : {}) };
...
async function executeCommitCreate(env: Env, operation: Extract<Operation, { kind: "commit.create" }>, token: string): Promise<OperationResult> {
  ...
  if (currentHead !== operation.expectedHeadSha) {
    const matchedCommit = await findPaginated(
      `${repoPath}/commits?sha=${encodeURIComponent(operation.branch)}`,
      token,
      "Commit idempotency lookup",
      (candidate) => {
        const message = record(candidate.commit) ? candidate.commit.message : undefined;
        const parentSha = Array.isArray(candidate.parents) && record(candidate.parents[0])
          ? candidate.parents[0].sha
          : undefined;
    ...
    if (matchedCommit && typeof matchedCommit.sha === "string") {
      return {
        status: "already-applied",
        githubId: matchedCommit.sha,
        url: `https://github.com/${operation.repository.owner}/${operation.repository.name}/commit/${matchedCommit.sha}`,
      };
    }
...
async function executePullOpen(env: Env, operation: Extract<Operation, { kind: "pull_request.open_draft" }>, token: string): Promise<OperationResult> {
  ...
  const repoPath = repositoryPath(operation);
  if (!hasExactOperationMarker(operation.body, operation.id)) {
    throw new GitHubOperationError("canonical_marker_missing", "Exact pull request body is missing its operation marker");
  }
  ...
  if (existing) {
    if (!record(existing.head) || existing.head.sha !== operation.expectedHeadSha || !record(existing.base) || existing.base.ref !== operation.base || existing.title !== operation.title || existing.draft !== operation.draft) throw new Error("Existing Gardener pull request does not match the approved operation");
    return { status: "already-applied", ...(positiveInteger(existing.number) ? { githubId: existing.number } : {}), ...(typeof existing.html_url === "string" ? { url: existing.html_url } : {}) };
  }
  ...
  const result = await jsonResponse(await github(`${repoPath}/pulls`, token, { method: "POST", body: JSON.stringify({ head: operation.head, base: operation.base, title: operation.title, body: operation.body, draft: operation.draft }) }), "Pull request creation");
...
function pullMatchesUpdate(
  pull: JsonRecord,
  operation: Extract<Operation, { kind: "pull_request.update" }>,
): boolean {
...
async function executePullUpdate(operation: Extract<Operation, { kind: "pull_request.update" }>, token: string): Promise<OperationResult> {
  ...
  if (pullMatchesUpdate(pull, operation)) {
    return { status: "already-applied", githubId: operation.pullNumber, ...(typeof pull.html_url === "string" ? { url: pull.html_url } : {}) };
  }
  const resumedAfterDraft = operation.draft !== undefined
    && operation.draft !== operation.expectedDraft
    && pull.draft === operation.draft;
  if (!resumedAfterDraft) assertPullEventState(pull, operation);

  let current = pull;
  if (operation.draft !== undefined && current.draft !== operation.draft) {
    if (typeof current.node_id !== "string") throw new Error("Pull request node id was missing");
    await setPullDraft(token, current.node_id, operation.draft);
    current = await pullRequest(repoPath, operation.pullNumber, token);
    assertPullRevision(current, operation);
    if (current.draft !== operation.draft) {
      throw new Error("GitHub did not apply the exact pull request draft state");
    }
  }
  ...
  if (Object.keys(patch).length) {
    current = await jsonResponse(await github(
      `${repoPath}/pulls/${operation.pullNumber}`,
      token,
      { method: "PATCH", body: JSON.stringify(patch) },
    ), "Pull request update") as JsonRecord;
    assertPullRevision(current, operation);
  }
  if (!pullMatchesUpdate(current, operation)) {
    throw new Error("GitHub did not apply the exact pull request update");
  }
  return { status: "applied", githubId: operation.pullNumber, ...(typeof current.html_url === "string" ? { url: current.html_url } : {}) };
}
...
export async function branchProtectionHash(protection: unknown): Promise<string> {
  const digest = await crypto.subtle.digest(
    "SHA-256",
    new TextEncoder().encode(canonicalJson(protection)),
  );
...
async function executePullMerge(operation: Extract<Operation, { kind: "pull_request.merge" }>, token: string): Promise<OperationResult> {
  ...
  if (await branchProtectionHash(protection) !== operation.expectedBranchProtectionHash) {
    throw new GitHubOperationError(
      "branch_protection_changed",
      "Branch protection changed after the operation was approved",
    );
  }
